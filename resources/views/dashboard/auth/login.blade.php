...
<head>
    ...
    <title>Masuk — Peminjaman Sarpras</title>
    ...
    <style>
        ...
        .d5 { animation-delay: 0.40s; opacity: 0; }
        .d6 { animation-delay: 0.48s; opacity: 0; }
    </style>
</head>
...
<body class="min-h-screen flex items-center justify-center bg-[#13132a] p-4">
    ...
    {{-- ═══════════════════════════════════════════════
         CARD WRAPPER
    ═══════════════════════════════════════════════ --}}
    <div class="flex w-full max-w-4xl min-h-[560px] rounded-3xl overflow-hidden shadow-[0_32px_80px_rgba(0,0,0,0.55)]">
        ...
        <div class="relative hidden md:flex flex-1 flex-col justify-between overflow-hidden
            ...
            <svg class="absolute bottom-0 left-0 right-0 w-full z-[1]"
                ...
                <defs>
                    <linearGradient id="lg-m3" x1="0" y1="0" x2="0" y2="1">
                        <stop offset="0%" stop-color="#312e81"/>
                        <stop offset="100%" stop-color="#1e1b4b"/>
                    </linearGradient>
                    <linearGradient id="lg-m2" x1="0" y1="0" x2="0" y2="1">
                        <stop offset="0%" stop-color="#4c1d95"/>
                        <stop offset="100%" stop-color="#2e1065"/>
                    </linearGradient>
                    <linearGradient id="lg-m1" x1="0" y1="0" x2="0" y2="1">
                ...
                <polygon points="0,200 70,110 150,165 230,85 320,150 420,65 500,130 500,280 0,280"
                         fill="url(#lg-m3)" opacity="0.7"/>
                <polygon points="0,235 90,150 180,195 265,105 355,165 460,100 500,145 500,280 0,280"
                         fill="url(#lg-m2)" opacity="0.88"/>
                <polygon points="-10,280 60,180 140,220 220,140 310,195 400,120 500,175 500,280"
                         fill="url(#lg-m1)"/>
                <rect x="0" y="0" width="500" height="280" fill="url(#lg-sky)"/>
            </svg>

            {{-- Stars --}}
            <div class="pointer-events-none absolute inset-0 z-[2]" aria-hidden="true">
                @foreach([['12%','15%'],['30%','8%'],['55%','20%'],['72%','6%'],['85%','18%'],
                          ['20%','35%'],['65%','30%'],['40%','12%']] as $s)
            ...
            {{-- Tagline --}}
            <div class="relative z-10 anim-left d2">
                <p class="text-3xl font-extrabold text-white leading-snug drop-shadow-lg">
                    Kelola Peminjaman<br>
                    <span class="text-violet-300">Sarana & Prasarana</span><br>
                    dengan Mudah
                </p>
                <p class="mt-3 text-sm text-white/50 font-medium">
                    Sistem informasi terpadu untuk sekolah modern
                </p>
            </div>
        </div>

        {{-- ─── RIGHT PANEL ─────────────────────────────── --}}
        <div class="flex flex-col justify-center bg-[#1c1d35] px-9 py-10 w-full md:w-[400px]">

            {{-- Header --}}
            <div class="mb-7 anim-up d2">
                <h1 class="text-[26px] font-extrabold text-slate-100 leading-tight">Masuk ke Akun</h1>
            ...
            <form action="{{ route('login') }}" method="POST" class="space-y-4">
                ...
                <div class="anim-up d4">
                    ...
                    <div class="relative">
                        ...
                        >
                        <button type="button"
                                onclick="togglePwd('pwd-login',this)"
                                class="absolute right-3.5 top-1/2 -translate-y-1/2
                                       text-slate-500 hover:text-slate-300 transition-colors">
                            <svg id="eye-login" width="18" height="18" viewBox="0 0 24 24" fill="none"
                ...
                {{-- Remember & forgot --}}
                <div class="flex items-center justify-between anim-up d5">
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" name="remember"
                               class="w-4 h-4 rounded border-slate-600 bg-[#252740]
            ...
            {{-- Divider --}}
            <div class="relative flex items-center gap-3 my-6 anim-up d6">
                <div class="flex-1 h-px bg-[#2d3154]"></div>
                <span class="text-xs text-slate-600 font-medium whitespace-nowrap">atau masuk dengan</span>
                <div class="flex-1 h-px bg-[#2d3154]"></div>
            </div>

            {{-- Social stub (UI only) --}}
            <div class="grid grid-cols-2 gap-3 anim-up d6">
                <button type="button"
